...
- Foto: Giusi_Journalsorriso_41.jpg su tutti e tre i campi sfondo_hook/sfondo_cta (da "Giusi - Casual Dez20 e Journal"); sfondo_quote = foto originale del template (MAFD12wZMVQ), stesso crop 1:1 dell'esempio reale
- Fix 2026-09-22 (tre passaggi):
  ...
  3. Finale: fascia info di pagina 1 = stessa foto hero (Journalsorriso), ma con crop estremo (zoom ~5x su una patch di sfondo) per simulare una sfocatura — l'API Canva MCP non espone un'operazione di blur/filtro vera, solo crop_media; vedi `reel_template.md`'s "Faking a blur" per il metodo. sfondo_quote confermato con crop 1:1 dell'esempio reale (invariato dal passaggio 2)
