...
- `generate-social-post` (step 0) syncs the pair automatically from Canva
  before every run, for any design in the `Social Media Automation` folder
  whose title carries Giusi's scheduling-date prefix (e.g. `10.9 at 9 pm
  ...`) — dating a design in Canva herself is the same approval signal, just
  read back from Canva instead of spoken in a session. This path is
  read-only against Canva and never touches Make; it exists so the reference
  set here (also what `social-critic` scores drafts against) doesn't lag
  behind what she's actually approved.
